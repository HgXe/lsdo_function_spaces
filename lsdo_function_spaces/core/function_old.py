...
class SurfaceMatrixProduct(Operation):
    def __init__(self, u_vec, v_vec, function, d_order):
        ...
        if d_order == 0:
            out_shapes = [(u_vec.shape[0], function.num_physical_dimensions)]
        elif d_order == 1:
            out_shapes = [(u_vec.shape[0], function.num_physical_dimensions), 
                          (u_vec.shape[0], function.num_physical_dimensions)]
        elif d_order == 2:
            out_shapes = [(u_vec.shape[0], function.num_physical_dimensions), 
                          (u_vec.shape[0], function.num_physical_dimensions), 
                          (u_vec.shape[0], function.num_physical_dimensions)]
        ...
        self.name = 'surface_matrix_product_order_' + str(d_order)
        self.function = function
        coefficients = function.coefficients.value
        if len(coefficients.shape) != 2:
            coefficients = coefficients.reshape((-1, function.num_physical_dimensions))
        self.coefficients = coefficients
    ...
    def compute_inline(self, u_vec, v_vec):
        ...
        if self.d_order == 0: 
            ...
            matrix_vv = self.function.space.compute_basis_matrix(parametric_coordinates, (0,2))
            matrix_uv = self.function.space.compute_basis_matrix(parametric_coordinates, (1,1))
            du2 = matrix_uu @ coefficients
            dv2 = matrix_vv @ coefficients
            duv = matrix_uv @ coefficients
            return du2, dv2, duv
    ...
    def evaluate_vjp(self, cotangents, u_vec, v_vec, *outputs):
        from .function import compute_smp

        if self.d_order == 0:
            matrix = outputs[0]
            u_derivative, v_derivative = compute_smp(u_vec, v_vec, self.function, d_order=1)
            if cotangents.check(u_vec):
                cotangents.accumulate(u_vec, csdl.sum(cotangents[matrix]*u_derivative, axes=(1,)))
            if cotangents.check(v_vec):
                cotangents.accumulate(v_vec, csdl.sum(cotangents[matrix]*v_derivative, axes=(1,)))
        elif self.d_order == 1:
            du = outputs[0]
            dv = outputs[1]
            du2, dv2, duv = compute_smp(u_vec, v_vec, self.function, d_order=2)
            if cotangents.check(u_vec):
                cotangents.accumulate(u_vec, csdl.sum(cotangents[du]*du2, axes=(1,)))
                cotangents.accumulate(u_vec, csdl.sum(cotangents[dv]*duv, axes=(1,)))
            if cotangents.check(v_vec):
                cotangents.accumulate(v_vec, csdl.sum(cotangents[du]*duv, axes=(1,)))
...
def compute_smp(u_vec, v_vec, function, d_order=0):
    u_vec = validate_and_variablize(u_vec, raise_on_sparse = False)
    v_vec = validate_and_variablize(v_vec, raise_on_sparse = False)
    op = SurfaceMatrixProduct(u_vec, v_vec, function, d_order)
...
class Function:
    ...
    def project_csdl(self, points:np.ndarray, direction:np.ndarray=None, grid_search_density_parameter:int=1, 
                max_newton_iterations:int=100, newton_tolerance:float=1e-6, plot:bool=False, force_reproject:bool=False, do_pickles=True) -> csdl.Variable:
        '''
        Projects a set of points onto the function. The points to project must be provided. If a direction is provided, the projection will find
        the points on the function that are closest to the axis defined by the direction. If no direction is provided, the projection will find the
        ...
        def compute_grad_hess(current_guess):
            ...
            rec.execute()
            return grad.value, hess.value
        
        rec.stop()

        # if direction is None:
        #     residual = csdl.sum(csdl.norm(displacements, axes=(1,)))
        # else:
        #     residual = csdl.sum(csdl.norm(csdl.cross(direction, displacements), axes=1))
        ...
        # prob = CSDLAlphaProblem(problem_name = 'projection', simulator=sim)

        # optimizer = PySLSQP(prob, solver_options={'maxiter':max_newton_iterations, 'acc':newton_tolerance})
